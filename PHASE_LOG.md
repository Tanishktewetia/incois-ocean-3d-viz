...
## Phase 18 - Immersive Ocean Research Landing Page

- **Status:** Complete.
- **Information architecture:** Added a dedicated public landing page at `/` and moved the existing scientific workspace to `/explorer`. Mission and comparison routes remain unchanged, while all internal calls-to-action now target the new Explorer path.
- **Experience:** Added a responsive, dark oceanographic visual system with an original procedural 3D data volume, scroll-reactive depth, model planes, current paths, instrument markers, a water-column story, capability cards, research workflow, and a focused launch path. The experience uses no generated stock image or external runtime asset.
- **Security and accessibility:** No image-generation API credential is stored in source or a temporary tracked file. Motion is disabled under `prefers-reduced-motion`; semantic headings, section labels, link focus behavior, and readable responsive layouts are retained.
- **Scope:** Existing data loading, Three.js Explorer rendering, camera behavior, scientific calculations, API calls, and upload behavior are unchanged.
- **Files changed:** `frontend/src/pages/LandingPage.jsx`, `frontend/src/App.jsx`, `frontend/src/pages/AboutPage.jsx`, `frontend/src/pages/ComparisonPage.jsx`, `frontend/src/styles.css`, `PHASE_LOG.md`.
- **Manual verification:** Open `/` at desktop, tablet, and mobile widths; confirm the hero depth composition, navigation, cards, and CTA remain readable without horizontal overflow. Scroll through the page and verify subtle volume movement, then enable reduced motion and confirm animation stops. Follow every Explorer CTA and confirm `/explorer` loads the existing scientific workspace; verify `/about` and `/comparison` navigation and back-links.
- **Automated validation:** Frontend production build passes with only Vite's existing non-fatal large-chunk advisory. `git diff --check` passes, and a repository scan confirms the supplied image-generation credential was not written to tracked source.
