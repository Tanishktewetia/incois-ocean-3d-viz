...
function TemperatureVolumeExplorerPage() {
  ...
  const [isothermContours, setIsothermContours] = useState(false);
  ...
  return <main className="temperature-explorer-page"><section className="page-intro"><div><p className="eyebrow">Dedicated figure · Phase 20</p><h1>Temperature Volume Explorer</h1></div><p className="intro-copy">A focused view of the Copernicus temperature field: locate a point, inspect its vertical profile, and sample a real cross-section through the water column.</p></section><div className="temperature-explorer-grid"><aside className="temperature-minimap"><div className="dedicated-heading"><div><p className="eyebrow">Map view</p><h2>India EEZ locator</h2></div><span className="map-instruction">{selectedLabel}</span></div><HeatmapCanvas dataSource={source} variable="thetao" onMapClick={selectMapPoint} /><div className="section-controls"><label htmlFor="slice-depth">Slice from top</label><select id="slice-depth" value={sliceDepth ?? ""} onChange={(event) => setSliceDepth(event.target.value === "" ? null : Number(event.target.value))}><option value="">Full water column</option>{depths.map((depth) => <option key={depth} value={depth}>{depth.toFixed(0)} m</option>)}</select><label htmlFor="temperature-vertical-scale">Vertical exaggeration <strong>{verticalExaggeration.toFixed(1)}×</strong></label><input id="temperature-vertical-scale" type="range" min="0.5" max="4" step="0.5" value={verticalExaggeration} onChange={(event) => setVerticalExaggeration(Number(event.target.value))} /><label className="toggle-row" htmlFor="temperature-isotherms"><span>Isotherm contours · 2 °C</span><input id="temperature-isotherms" type="checkbox" checked={isothermContours} onChange={(event) => setIsothermContours(event.target.checked)} /></label><small>Slice levels are snapped to the real model. Contours are computed from loaded temperature values.</small></div></aside><section className="temperature-profile-panel"><div className="dedicated-heading"><div><p className="eyebrow">Point profile</p><h2>Temperature vs depth</h2></div>{profile && <span>{profile.latitude.toFixed(2)}°N · {profile.longitude.toFixed(2)}°E</span>}</div><ProfileChart profile={profile} /></section><section className="temperature-transect-panel"><div className="dedicated-heading"><div><p className="eyebrow">Transect panel</p><h2>Temperature cross-section</h2></div><span>Click two map points</span></div><TransectChart transect={transect} /></section></div>{error && <p className="control-error" role="alert">{error}</p>}<OceanScene3D dataSource={source} initialVariable="thetao" presentation onDataSelect={selectPoint} sliceDepth={sliceDepth} dedicatedVerticalExaggeration={verticalExaggeration} dedicatedIsothermContours={isothermContours} /></main>;
}
